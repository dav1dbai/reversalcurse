...
LORA_RANKS=(8 16 32 64 128 256 512 1024) # Define LoRA ranks to iterate over
...
for rank in "${LORA_RANKS[@]}"; do
    ...
    echo "Command: python LORA_finetuner.py --lora_rank \"$rank\" --base_model_name \"$BASE_MODEL_NAME\""
    
    python LORA_finetuner.py --lora_rank "$rank" --base_model_name "$BASE_MODEL_NAME"
    ...
    echo "Command: python inference.py --model_path \\"$TUNER_OUTPUT_MODEL_PATH\\" --base_model_name \\"$BASE_MODEL_NAME\\""
    ...
    python inference.py --model_path "$TUNER_OUTPUT_MODEL_PATH" --base_model_name "$BASE_MODEL_NAME"
